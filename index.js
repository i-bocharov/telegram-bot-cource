...
const start = () => {
  ...
  bot.on('message', async msg => {
    ...
    if (text === '/info') {
        return bot.sendMessage(chatId, `Тебя зовут ${msg.from.first_name} ${msg.from.last_name}, и твой ID: ${msg.from.id}`);
    }
    if (text === '/game') {
  ...
  bot.on('callback_query', async msg => {
    ...
    if (data === chats[chatId]) {
      ...
      return bot.sendMessage(chatId, `К сожалению ты не угадал, бот загадал число ${chats[chatId]}. Попробуй еще раз!`, againOptions);
    }
  });
}
